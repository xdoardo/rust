...
pub trait Capability<'a>: core::marker::PointerLike + Sized {
    ...
    /// Seal this capability. Returns `None` if `key` can't seal this capability.
    unsafe fn seal<K: core::marker::PointerLike>(self, key: K) -> Option<Self>;

    /// Unseal this capability. Returns `None` if `key` can't unseal this capability, or if the
    /// given capability is not sealed.
    unsafe fn unseal<K: core::marker::PointerLike>(self, key: K) -> Option<Self>;
...
impl<'a, T: core::marker::PointerLike + Copy> Capability<'a> for T {
    ...
    #[inline(always)]
    unsafe fn seal<K: crate::marker::PointerLike>(self, key: K) -> Option<Self> {
        let ret = unsafe { crate::intrinsics::cheri::cheri_seal(self, key) };
        if ret.is_valid() { Some(ret) } else { None }
    }

    #[inline(always)]
    unsafe fn unseal<K: crate::marker::PointerLike>(self, key: K) -> Option<Self> {
        let ret = unsafe { crate::intrinsics::cheri::cheri_unseal(self, key) };
        if ret.is_valid() { Some(ret) } else { None }
    }
